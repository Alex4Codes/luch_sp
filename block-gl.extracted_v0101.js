!(function (t) {
  ...
  // ============================================================
  // НАСТРОЙКИ АНИМАЦИИ — все управляемые параметры здесь
  // ============================================================
  const SETTINGS = {

    // Масштаб рендера относительно размера экрана
    // Мобильные: 0.35 — меньше нагрузка на GPU
    // Десктоп:   0.5  — лучше качество картинки
    ...
    motion: {
      // Скорость следования курсора/пальца (0.0–1.0)
      // 0.05 = очень плавно и инертно
      // 0.5  = быстро, почти без задержки
      pointerLerp: 0.1,

      // Скорость волны sine (отрицательное = инверсия)
      waveLerp: -1,

      // Скорость перехода угла shatter (0.001–0.05)
      // Меньше = плавнее поворот кристаллов
      shatterLerp: 0.005,

      // Скорость поворота лучей bokeh (0.001–0.05)
      rayLerp: 0.005,

      // Диапазон угла shatter при движении (0.0–0.1)
      shatterRange: 0.01,

      // ── АВТОАНИМАЦИЯ НА МОБИЛЬНЫХ ──────────────────────────

      // Общая скорость автодвижения pointer
      // 0.2 = очень медленно
      // 0.4 = среднее (рекомендуется)
      // 1.0 = быстро
      autoSpeed: 0.4,

      // Амплитуда движения по X (0.1–0.5)
      // 0.5 = движение по всему экрану
      // 0.2 = движение в центральной зоне
      autoAmplitudeX: 0.35,

      // Амплитуда движения по Y (0.1–0.5)
      autoAmplitudeY: 0.35,

      // Частота синусоиды по X — влияет на форму траектории
      // Рекомендуемые пары X/Y:
      // 1.0 / 1.0 = круговая траектория
      // 1.0 / 2.0 = восьмёрка
      // 1.0 / 3.0 = трилистник
      // 2.0 / 3.0 = сложная фигура Лиссажу
      autoFreqX: 1.0,

      // Частота синусоиды по Y
      autoFreqY: 1.7,

      // Сдвиг фазы по Y в радианах — смещает фигуру траектории
      // Math.PI / 2 = сдвиг на четверть периода
      // Math.PI / 3 = сдвиг на треть периода (рекомендуется)
      autoPhaseY: Math.PI / 3,

      // Задержка возврата к авторежиму после касания (мс)
      // 2000 = 2 секунды
      // 5000 = 5 секунд
    ...
    background: {
      // Основной цвет фона в hex
      color: "#fff9b2",

      // Второй цвет для градиента фона в hex
      color2: "#ffcc00",

      // Позиция центра градиента [x, y] от 0.0 до 1.0
      // [0.5, 0.5] = центр экрана
      // [0.0, 0.0] = левый нижний угол
      gradientPos: [0.08, 0.12],

      // Мощность/резкость градиента (0.1–2.0)
      // 0.1 = очень мягкий градиент
      // 2.0 = резкий переход
      gradientPower: 0.5,
    },
    ...
    vignette: {
      ...
      // Радиус светлой зоны (0.1–1.0)
      // 0.3 = виньетка занимает большую часть экрана
      // 0.8 = виньетка только по самым краям
      radius: 0.55,

      // Мягкость края виньетки (0.0–2.0)
      // 0.0 = резкий край
      // 2.0 = очень плавный переход
      falloff: 1,

      // Смещение виньетки от яркости (0.0–1.0)
      displace: 0,

      // Интенсивность смешивания виньетки (1–5)
      mix: 3,

      // Угол поворота виньетки (0.0–1.0 соответствует 0–360°)
      angle: 0,

      // Скос формы виньетки (0.0–1.0)
      // 0.5 = симметричная форма
      // 0.0 или 1.0 = вытянутая форма
      skew: 0.54,
    },

    sine: {
      // Частота волны (0.1–1.0)
      // 0.1 = редкие широкие волны
      // 1.0 = частые узкие волны
      frequency: 0.35,

      // Амплитуда волны (0.5–2.0)
      // Больше = сильнее искажение
      amplitude: 1.18,

      // Спад волны от центра к краям (0.0–1.0)
      falloff: 0.5,

      // Направление волны
      // 0 = горизонтальные волны
      // 1 = вертикальные волны
      rotation: 0,

      // Начальная фаза волны (0.0–1.0)
      phase: 0,

      // Скорость анимации волны (0.1–2.0)
      speed: 0.7,

      // Радиус зоны влияния волны (0.0–1.0)
      // 0.0 = волна только в точке курсора
      // 1.0 = волна на весь экран
      mixRadius: 1,

      // Привязка к курсору/пальцу
      // 0 = волна фиксирована по центру
      // 1 = волна следует за курсором
      trackMouse: 1,
    },

    shatter: {
      // Масштаб ячеек кристаллизации (0.1–2.0)
      // 0.3 = очень мелкие кристаллы
      // 1.5 = крупные кристаллы
      scale: 0.9,

      // Интенсивность смещения кристаллов (0.5–3.0)
      amount: 1.3,

      // Базовый угол кристаллов в градусах (-180–180)
      angle: -30,

      // Радиус зоны эффекта (0.5–2.0)
      radius: 1,

      // Скос кристаллов (0.0–1.0)
      skew: 1.0,

      // Радиус смешивания (0.0–1.0)
      mixRadius: 1,

      // Тип сглаживания движения
      // 0 = линейное
      // 1 = ease (плавное)
      easing: 0,
    },
    ...
    bokeh: {
      // Радиус размытия боке
      // 0.0     = эффект отключён
      // 0.01    = лёгкое размытие
      // 0.05    = сильное размытие
      // На мобильных отключён автоматически для производительности
      radius: isMobile ? 0 : 0.03,

      // Тип распределения боке (0.0–1.0)
      // 0.0 = равномерное размытие по всему экрану
      // 1.0 = размытие только по краям (тилт-шифт эффект)
      tilt: 0.9,

      // Радиус зоны влияния (0.0–1.0)
      mixRadius: 1,
    },

    output: {
      // Цвет финального наложения в hex
      // Влияет на общий тон итоговой картинки
      color: "#fcffe8",
    },
  };
